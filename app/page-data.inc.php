...
Class PageData {
  ...
  static function create_asset_collections($page) {
    ...
    # page.images
    $page->images = Helpers::list_files($page->file_path, '/(?<!thumb|cover|_lge|_sml)\.(gif|jpg|png|jpeg)$/i', false);
    # page.numbered_images
    $page->numbered_images = Helpers::list_files($page->file_path, '/^\d+[^\/]*(?<!thumb|cover|_lge|_sml)\.(gif|jpg|png|jpeg)$/i', false);
    # page.video
    $page->video = Helpers::list_files($page->file_path, '/\.(mov|mp4|m4v)$/i', false);
    # page.cover |||| __ZELF TOEGEVOEGD__ ||||
    $page->cover = Helpers::list_files($page->file_path, '/(cover|cvr)\.(cover|cvr|gif|jpg|png|jpeg)$/i', false); #'/(?<!cover)\.(gif|jpg|png|jpeg)$/i', false);
